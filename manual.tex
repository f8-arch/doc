...
\begin{itemize}
\item An efficient stackpointer-relative addressing mode for efficient handling of local variables
\item A unified address space for efficient pointer access
\item Having a few data / pointer registers for temporary storage
\item Hardware multithreading and support for atomics to replace peripheral hardware
\end{itemize}
...
\begin{itemize}
\item Attempts to write via null pointers reset the f8 (via an I/O register at address 0x0000).
\item Attempts to execute 0-initialized memory reset the f8 (via the \hyperref[inst:trap]{trap instruction} that has opcode 0x00).
\item A simple  \hyperref[periph:watchdog]{watchdog} can reset the f8.
\end{itemize}
...
After reset, \texttt{pc} has the value 0x4000 the value of the other registers mentioned in this section after reset is unspecified.
...
\texttt{altacc1} & alternative accumulator \texttt{xh} instead of \texttt{xl} & 1 \\
\texttt{altacc2} & alternative accumulator \texttt{yl} instead of \texttt{xl}, \texttt{z} instead of \texttt{y} & 2 \\
\texttt{altacc4} & alternative accumulator \texttt{yh} instead of \texttt{xl}, \texttt{z} instead of \texttt{y} & 2 \\
\texttt{altacc3} & alternative accumulator \texttt{zl} instead of \texttt{xl}, \texttt{x} instead of \texttt{y} & 2 \\
\texttt{altacc5} & alternative accumulator \texttt{zh} instead of \texttt{xl} & 2
\end{tabular}
...
\section{8-bit 2-operand instructions}
...
\begin{tabular}{l l l}
Assembler code              & Operation                              & f8l \\
\texttt{adcw y, op16\_2}    & \texttt{y = y + op16\_2 + c}           & No \\
\texttt{adcw op16\_2ni, y} & \texttt{op16\_2ni = op16\_2ni + y + c} & No
\end{tabular}
...
\begin{tabular}{l l l}
Assembler code               & Operation                          & f8l \\
\texttt{addw y, op16\_2}     & \texttt{y = y + op16\_2}           & No \\
\texttt{addw op16\_2ni, y}   & \texttt{op16\_2ni = op16\_2ni + y} & No
\end{tabular}
...
\begin{tabular}{l l l}
Assembler code             & Operation                          & f8l \\
\texttt{orw y, op16\_2}    & \texttt{y = y | op16\_2}           & No \\
\texttt{orw op16\_2ni, y}  & \texttt{op16\_2ni = op16\_2ni | y} & No
\end{tabular}
...
\begin{tabular}{l l l}
Assembler code              & Operation                                & f8l \\
\texttt{sbcw y, op16\_2ni}  & \texttt{y = y + \~{}op16\_2ni + c}       & No \\
\texttt{sbcw op16\_2ni, y}  & \texttt{op16\_2 = \~{}op16\_2ni + y + c} & No
\end{tabular}
...
\begin{tabular}{l l l}
Assembler code              & Operation                                & f8l \\
\texttt{subw y, op16\_2ni}  & \texttt{y = y + \~{}op16\_2ni + 1}       & No \\
\texttt{subw op16\_2ni, y}  & \texttt{op16\_2 = \~{}op16\_2ni + y + 1} & No
\end{tabular}
...
\begin{tabular}{l l l}
Assembler code              & Operation                             & f8l \\
\texttt{xorw y, op16\_2}    & \texttt{y = y \^{} op16\_2}           & No \\
\texttt{xorw op16\_2ni, y}  & \texttt{op16\_ni2 = op16\_2ni \^{} y} & No
\end{tabular}
...
Initializing or setting an object, or parts thereof, to 0 is common, so having a dedicated instruction is worth it vs. using \texttt{ld}.
...
8-bit stack parameters can be passed easily via this instruction. Registers can be saved temporarily (e.g. for the duration of a function call, or in the middle of a complex computation). Not affecting flags makes the instruction more useful for saving registers in the middle of a long addition\slash subtraction\slash comparison\slash multiplication.
...
Flags according to old \texttt{(z)}.
...
This instruction allows the efficient implementation of explicit casts of 8-bit numbers to bool and, together with the \texttt{xor} instruction, of the negation operator for 8-bit numbers.
...
Registers that were saved temporarily via a \texttt{push} can be restored by this instruction. Not affecting flags makes the instruction more useful for restoring registers after a comparison before a conditional jump, or in the middle of a long addition\slash subtraction\slash multiplication.
...
Getting the hardware thread number efficiently is useful for implementing thread-local storage. While \texttt{thrd} will not be a common instruction is typical programs, the alternative is doing a search for the current value of \texttt{sp} in a list of stack pointer ranges, which would be quite inefficient.
...
This instruction allows the efficient implementation of explicit casts of 16-bit numbers to bool and, together with the \texttt{xor} instruction, of the negation operator for 16-bit numbers.
...
Negation is a common special case of multiplication.
...
This instruction is useful to restore 16-bit registers that were saved temporarily via \texttt{pushw}. It is also a code-size efficient way of adjusting the stack pointer by 2 (but does a memory read).
...
\texttt{sllw altacc16}          & \texttt{c = altacc16 \& (0x8000 >> 15); altacc16 = altacc16 << 1}             & No \\
\texttt{sllw y, xl}             & \texttt{y = y << xl}                          & No \\
...
\texttt{call \#ii} ignores all flags, changes no flags, not even reserved ones.
...
Calling and returning from functions using a return address on the stack is common. This instruction helps implement it efficiently. Not affecting flags, not even reserved ones, makes \texttt{call \#ii} suitable for as a software interrupt.
...
This instruction can be used to implement while loops instead of using \texttt{dec} followed by \texttt{jr nz}. Not affecting flags makes it suitable for implementing arbitrary-length arithmetic (\texttt{dec} would not preserve the carry flag, thus complicating its use). The choice of operands is motivated by the use-case of arbitrary-length multiplications via \texttt{mad}.
...
A jump instruction that can reach any target is very useful to implement control-flow. Not affecting flags, not even reserved ones, makes \texttt{jp \#ii} instruction suitable for use at the interrupt vector.
...
\subsection{trap}\label{inst:trap}
...
Some bugs, including many security-relevant ones can lead to the execution of code from memory used for data. Many exploits actually rely on data commonly being zero, and \texttt{nop} having opcode 0. By making opcode 0 a \texttt{trap} instruction, we can mitigate the impact of such bugs, and make them easier to debug.


\section{Non-instructions}

A 16-bit bitwise and \texttt{andw} would not be as useful as \texttt{orw} and \texttt{xorw}: known 0x00 or 0xff bytes are more common for bitwise and, so the compiler will often use \texttt{ld}, \texttt{ldw}, \texttt{clr} and \texttt{clrw}, and handle the rest with 8-bit \texttt{and}.
...
\hline\hline
0x & trap & sub xl, mm & sub xl (n, sp) & sub xl (nn, z) & sub xl, zl & sub xl, xh & sub xl, yl & sub xl, yh &  & sbc xl, mm & sbc xl (n, sp) & sbc xl (nn, z) & sbc xl, zl & sbc xl, xh & sbc xl, yl & sbc xl, yh \\
\hline
1x & add xl, \#i & add xl, mm & add xl (n, sp) & add xl (nn, z) & add xl, zl & add xl, xh & add xl, yl & add xl, yh & adc xl, \#i & adc xl, mm & adc xl (n, sp) & adc xl (nn, z) & adc xl, zl & adc xl, xh & adc xl, yl & adc xl, yh \\
\hline
2x & cp xl, \#i & cp xl, mm & cp xl (n, sp) & cp xl (nn, z) & cp xl, zl & cp xl, xh & cp xl, yl & cp xl, yh & or xl, \#i & or xl, mm & or xl (n, sp) & or xl (nn, z) & or xl, zl & or xl, xh & or xl, yl & or xl, yh \\
\hline
3x & and xl, \#i & and xl, mm & and xl (n, sp) & and xl (nn, z) & and xl, zl & and xl, xh & and xl, yl & and xl, yh & xor xl, \#i & xor xl, mm & xor xl (n, sp) & xor xl (nn, z) & xor xl, zl & xor xl, xh & xor xl, yl & xor xl, yh \\
\hline
4x & srl mm & srl (n, sp) & srl xl & srl (n, y) & sll mm & sll (n, sp) & sll xl & sll (n, y) & rrc mm & rrc (n, sp) & rrc xl & rrc (n, y) & rlc mm & rlc (n, sp) & rlc xl & rlc (n, y) \\
\hline
5x & inc mm & inc (n, sp) & inc xl & inc (n, y) & dec mm & dec (n, sp) & dec xl & dec (n, y) & crl mm & crl (n, sp) & crl xl & crl (n, y) & tst mm & tst (n, sp) & tst xl & tst (n, y) \\
\hline
6x & push mm & push (n, sp) & push xl & push (n, y) & jp \#ii & jp y & call \#ii & call y &  &  &  &  &  &  &  &  \\
\hline
7x & ldw y, sp & subw y, mm & subw y, (n, sp) & subw y, x & ldw ((n,sp)), y & sbcw y, mm & sbcw y, (n, sp) & sbcw y, x & addw y, \#ii & addw y, mm & addw y, (n, sp) & addw y, x & adcw y, \#ii & adcw y, mm & adcw y, (n, sp) & adcw y, x \\
\hline
8x & ld xl, \#i & ld xl, mm & ld xl, (n, sp) & ld xl, (nn, z) & ld xl, (y) & ld xl, (n, y) & ld xl, xh & ld xl, yl & ld xl, yh & ld xl, zl & ld xl, zh & ld mm, xl & ld (n, sp), xl & ld (nn, z), xl & ld (y), xl & ld (n, y), xl \\
\hline
9x & push \#i & xch xl, (n, sp) & xch xl, (y) & xch yl, yh & altacc4 & rot xl, \#i & sra xl & da xl & bool xl & pop xl & thrd xl & cax (y), zl, xl & swapop & altacc1 & altacc2 & altacc3 \\
\hline
ax & clrw mm & clrw (n, sp) & clrw (nn, z) & clrw y & incw mm & incw (n, sp) & incw (nn, z) & incw y & adcw mm & adcw (n, sp) & adcw (nn, z) & adcw y & sbcw mm & sbcw (n, sp) & sbcw (nn, z) & sbcw y \\
\hline
bx & pushw mm & pushw (n, sp) & pushw (nn, z) & pushw y & tstw mm & tstw (n, sp) & tstw (nn, z) & tstw y & msk (y), xl, \#i & mul y & ret & reti & mad x, mm, yl & mad x, (n, sp), yl & mad x, (nn, z), yl & mad x, (z), yl \\
\hline
cx & ldw y, \#ii & ldw y, mm & ldw y, (n, sp) & ldw y, (nn, z) & ldw y, (n, y) & ldw y, (y) & ldw y, x & ldw y, \#d & ldw mm, y & ldw (n, sp), y & ldw (nn, z), y & ldw x, y & ldw z, y & ldw (y), x & ldw (n, y), x & ldwi (n, y), (z) \\
\hline
dx & jr \#d & dnjnz yh, \#d & jrz \#d & jrnz \#d & jrc \#d & jrnc \#d & jrn \#d & jrnn \#d & altacc5 & jrno \#d & jrsge \#d & jrslt \#d & ldw y, z & jrsle \#d & ldw x, (y) & jrle \#d \\
\hline
ex & srlw y & sllw y & rrcw y & rlcw y & sraw y & sllw y, xl & rrcw (n, sp) & rlcw (n, sp) & pushw \#ii & popw y & addw sp, \#d & addw y, \#d & xch f, (n, sp) & ldi (n, y), (z) & sex y, xl & zex y, xl \\
\hline
fx & orw y, \#ii & orw y, mm & orw y, (n, sp) & orw y, x & xchw x, (y) & xchw y, (n, sp) & incnw y & decw (n, sp) & cpw y, \#ii & caxw (y), z, x & negw y & boolw y & xorw y, \#ii & xorw y, mm & xorw y, (n, sp) & xorw y, x 
\end{tabular}
\end{adjustbox}
\end{center}
...
\section{Watchdog and Reset}\label{periph:watchdog}
...
The system is reset when a power-on reset happens, the watchdog counter register reaches 0xffff, the \hyperref[inst:trap]{trap instruction} is executed, or the byte at memory address 0x0000 is written.
